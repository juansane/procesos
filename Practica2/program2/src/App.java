...
public class App {
        ...
        private static void searchCharString() {
            ...
            try {
                ...
                for (int i = 0; i < pathList.size(); i++) {
                    File file = new File(pathList.get(i));
                    try {
                        ProcessBuilder processBuilder = new ProcessBuilder("java.exe", "-cp", PROGRAM1_PATH, "App", pathList.get(i), charString);
    ...
    /* -- Replace String -- */
        ...
        private static void replaceCharString() {
            ...
            for (int i = 0; i < pathList.size(); i++) {
                try {
                        ProcessBuilder processBuilder = new ProcessBuilder("java.exe", "-cp", PROGRAM1_PATH, "App", pathList.get(0), charString, replaceCharString);
                        start = processBuilder.start();
                        try(BufferedReader ipBuf = new BufferedReader(new InputStreamReader(start.getInputStream()))) {
                            String line = null;
                            while ((line = ipBuf.readLine()) != null)
            ...
            try {
                int i = 0;
                Process pb = new ProcessBuilder("java.exe", "-cp", PROGRAM1_PATH, "App", pathList.get(i), charString, replaceCharString).start();
